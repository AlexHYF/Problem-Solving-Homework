...
Running Time $O(V^2)$
\section*{22.2-5}
Since \id{u.d} = d(\id{s,v}), the distance between two node is independent from the way the graph is presented, so \id{u.d} is in dependent from the order in the adjacency-list.\\
 ...
 \section*{22.3-7}
 To do this, an extra pointer is needed for all the nodes which points to the next node to discover, this pointer is set to the first element of the adjacency-list of each node at first.\\
 ...
 \li \func{PUSH(\id{S,u})}
  ...
  \li		Set \id{t.p} to the next node in the adjacency-list of t. \End
  \End
 \end{codebox}
 \section*{22.3-8}
...
\id{u}, so there isn't a \id{(v,u)} path. And for \id{u}, it only be connected with verticces after \id{v} and also none of them are directly connected to \id{v}. So there isn't a \id{(u,v)} path. So this graph is
